...
type MessagePayload = { phoneNumbers: string[]; message: string; userName: string }[];
...
export const createMessagePayload = (selectedPledgers: Pledger[], events: Event): MessagePayload => {
    ...
    return selectedPledgers.map((pledger) => {
        ...
        const userName = import.meta.env.VITE_AT_SANDBOX_USERNAME;

        return {phoneNumbers, message, userName};
    });
}
